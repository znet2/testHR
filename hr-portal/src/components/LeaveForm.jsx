...
export default function LeaveForm({ type, onClose, onToast, userName }) {
  ...
  async function processFile(file) {
    ...
    if (file.size > MAX_FILE_MB * 1024 * 1024) {
      ...
      setSizeError(true);
      onToast(`ไฟล์ใหญ่เกิน ${MAX_FILE_MB} MB`, 'error');
      return;
    }
    setSizeError(false);
    try {
  ...
  async function handleSubmit(e) {
    ...
    params.append('name',      userName || fd.get('name') || '');
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <input name="name" value={userName || ''} readOnly
          className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-gray-100 text-gray-700 cursor-default" />
      </div>
      ...
      {isSick && (
        <>
          <div>
            ...
            <select name="hasCert" value={hasCert}
              onChange={e => { setHasCert(e.target.value); if (e.target.value === 'no') removeImg(); }}
